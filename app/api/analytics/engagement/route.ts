...
// GET /api/analytics/engagement?period=30d
// Returns engagement metrics from published posts
async function handler(req: NextRequest, context: { userId: string }): Promise<NextResponse> {
  try {
    ...
    const metrics = await analyticsService.getEngagementMetrics(context.userId, period)
