...
import { pipelinePersonaVideoSchema } from '@/lib/validations/pipeline';
...
export const POST = withAuth(async (req, { userId }) => {
  try {
    ...
    const validated = pipelinePersonaVideoSchema.parse(body);
    ...
    const result = await pipelineService.generatePersonaVideo(userId, replicateKey, validated);
    ...
    const message = error instanceof Error ? error.message : 'Falha no pipeline de vídeo';
