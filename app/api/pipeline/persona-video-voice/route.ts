...
import { pipelinePersonaVideoWithVoiceSchema } from '@/lib/validations/pipeline';
...
export const POST = withAuth(async (req, { userId }) => {
  try {
    ...
    const validated = pipelinePersonaVideoWithVoiceSchema.parse(body);
    ...
    const result = await pipelineService.generatePersonaVideoWithVoice(
      userId,
      replicateKey,
      validated
    );
    ...
    const message = error instanceof Error ? error.message : 'Falha no pipeline de vídeo com narração';
