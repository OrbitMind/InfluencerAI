...
async function main() {
  ...
  // ============================================
  // UGC Templates (Sprint 10)
  // ============================================
  console.log('\nSeeding UGC templates...');
  ...
  const ugcTemplates = [
    {
      ...
      category: 'ugc',
      icon: '📦',
      imagePromptTemplate: '{{persona_base}}, holding a {{product_name}} box with excited expression, unboxing moment, natural lighting, close-up shot, authentic UGC style',
      videoPromptTemplate: '{{persona_base}}, opening {{product_name}} packaging with genuine excitement, revealing the product, talking to camera, handheld feel, authentic home setting, UGC style, {{camera_movement}}',
      narrationTemplate: 'Gente, acabou de chegar o meu {{product_name}} e eu precisei fazer esse vídeo aqui pra vocês! Olha que incrível isso aqui...',
    ...
    },
    {
      ...
      category: 'ugc',
      icon: '⭐',
      imagePromptTemplate: '{{persona_base}}, holding {{product_name}}, honest review pose, neutral background, professional yet approachable, direct eye contact, authentic UGC style',
      videoPromptTemplate: '{{persona_base}}, reviewing {{product_name}} with detailed demonstration, showing product features, honest talking head style, good lighting, static camera for credibility',
      narrationTemplate: 'Faz {{time_used}} que estou usando o {{product_name}} e hoje vou dar minha avaliação honesta. Pontos positivos, negativos, e se vale a pena comprar.',
    ...
    },
    {
      ...
      category: 'ugc',
      icon: '💬',
      imagePromptTemplate: '{{persona_base}}, warm confident smile, testimonial portrait, soft natural light, relatable authentic expression, UGC lifestyle',
      videoPromptTemplate: '{{persona_base}}, sharing personal story about {{transformation}}, emotional and genuine talking head, warm home background, slow zoom in, authentic testimonial style',
      narrationTemplate: 'Antes de usar {{product_name}}, eu {{before_situation}}. Depois de {{time_period}}, {{after_result}}. Não acreditava que ia funcionar mas...',
      defaultCameraMovement: 'zoom_in',
      variables: [
    ...
    },
    {
      ...
      category: 'ugc',
      icon: '🤳',
      imagePromptTemplate: '{{persona_base}}, POV selfie angle, {{activity}} setting, first-person perspective, candid authentic moment, natural lighting',
      videoPromptTemplate: 'POV: {{pov_scenario}}, first person perspective, {{persona_base}} visible in frame or implied, handheld shaky cam, immersive and relatable, {{camera_movement}}',
    ...
    },
    {
      ...
      category: 'ugc',
      icon: '🌅',
      imagePromptTemplate: '{{persona_base}}, morning routine vibe, {{time_of_day}} setting, lifestyle photography, authentic daily life moment, soft warm lighting',
      videoPromptTemplate: '{{persona_base}}, daily routine montage style, naturally using {{product_name}} throughout the day, authentic lifestyle shots, variety of angles and settings, vlog aesthetic',
      narrationTemplate: 'Vem comigo num dia na minha vida com {{product_name}}! Da manhã até a noite, como uso esse produto no meu dia a dia.',
    ...
    },
    {
      ...
      category: 'ugc',
      icon: '✨',
      imagePromptTemplate: '{{persona_base}}, split screen composition, before and after transformation, {{transformation_type}}, dramatic lighting change, compelling visual contrast',
      videoPromptTemplate: '{{persona_base}}, before and after reveal, starting with {{before_state}} then transforming to {{after_state}} using {{product_name}}, satisfying reveal moment, dramatic transition',
      narrationTemplate: 'Antes: {{before_state}}. Depois de usar {{product_name}}: {{after_state}}. A diferença é real!',
    ...
    },
    {
      ...
      category: 'ugc',
      icon: '💄',
      imagePromptTemplate: '{{persona_base}}, getting ready scene, mirror selfie angle, beauty products visible, morning routine aesthetic, warm bathroom lighting, authentic GRWM style',
      videoPromptTemplate: '{{persona_base}}, get ready with me style video, applying {{product_category}} products step by step, talking to camera between steps, close-up product shots, natural lighting, tilt down movement',
      narrationTemplate: 'Vem se arrumar comigo! Hoje vou mostrar minha rotina com {{product_name}}. Passo a passo, sem segredos.',
  ...
  for (const template of ugcTemplates) {
    await prisma.campaignTemplate.upsert({
      where: { slug: template.slug },
      update: {
        name: template.name,
        ...
        defaultCameraMovement: template.defaultCameraMovement ?? null,
        variables: template.variables,
        isSystem: true,
        isActive: true,
      },
  ...
  console.log(`\nSeeded ${ugcTemplates.length} UGC templates.`);
}
